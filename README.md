...
7. After power-on, hold the cane at its normal use angle for about 2.2 s while the firmware learns and settles the compensated ground baseline. A `-9 cm` compensated change reports `ground_step` with `direction=up`; `+11 cm` reports `ground_step` with `direction=down`; `+30 cm` reports `ground_drop`. A hand sweep clears its candidate and needs `250 ms` of stable normal use before a new step candidate; absolute down distance alone never raises a step alert. Each confirmed step/drop gives one local cue while its new ground baseline is learned, rather than repeating continuously.
