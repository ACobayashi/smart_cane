...
4. 在盲杖前方放置障碍物。固件使用 `100 ms` 采样周期，VL53L1X `timing budget` 为 `20 ms`。检测到新的风险状态后：
...
5. 保持盲杖位置不变，并维持相同障碍状态。对于**同一地点、同一类型的风险**，系统不会持续重复：
...
系统会根据新的空间关系重新给出绕行方向，并允许记录新的风险事件。

7. 将盲杖保持在正常握持姿态约 `0.7 s`，让固件学习姿态补偿后的地面基线。之后：

* 补偿后高度变化 `-9 cm`：上报 `ground_step`，`direction=up`
* 补偿后高度变化 `+11 cm`：上报 `ground_step`，`direction=down`
* 补偿后高度变化 `+30 cm`：上报 `ground_drop`

**绝对下视距离本身不会再直接触发台阶告警。**

8. 长按触摸电极 `E1` 或执行 `mark`。后端会在当前位置记录 `user_mark` 风险点。

9. 执行 `path`，打印本地路线 / 风险环形缓冲区。

10. 将 `SMARTCANE_DEVICE_ID` 改为 `cane_002` 后重新烧录，并执行 `nearby`。第二根盲杖会读取附近历史风险统计并融合到本地风险判断。

11. 短按物理按键或执行 `btn`：固件上传 `voice_request`，盲人 Android 端进入语音交互模式；陪护人端不会收到这个普通语音请求。

12. 按住物理按键 `2 s` 或执行 `sos`：盲杖振动、蜂鸣、打印 SOS，并上传 `sos`；后端会将其与 `fall_detected` 区分处理。

13. 跌倒检测时，将 BMI270 模块轻放或倾斜到软垫上，并保持短暂侧躺：

* 固件只使用蜂鸣器，不使用普通障碍振动提示；
* 本地先进入横躺确认与锁定阶段；
* 只有确认正式跌倒后才上传 `fall_detected`；
* 只有恢复到稳定正常握杖姿态后，跌倒锁定才会解除。

串口命令列表详见 `firmware/smartcane_arduino/README.md`。
