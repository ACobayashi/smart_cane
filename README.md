...
7. After power-on, hold the cane at its normal use angle for about 2.2 s while the firmware learns and settles the compensated ground baseline. A `-9 cm` compensated change reports `ground_step` with `direction=up`; a `+50 cm` compensated change reports `ground_step` with `direction=down`; `+70 cm` reports `ground_drop`. The asymmetric downward threshold prevents a small upward hand sweep of the diagonal cane from becoming a false downstairs alert. A hand sweep clears its candidate and needs `250 ms` of stable normal use before a new step candidate; absolute down distance alone never raises a step alert. Each confirmed step/drop gives one local cue while its new ground baseline is learned, rather than repeating continuously.
